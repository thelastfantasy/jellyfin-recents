...
- **帧导出与拼接**：点击播放器 OSD 中的帧导出按钮，浏览当前播放位置附近的关键帧；可选择帧导出为 GIF/WebP 动画，或拼接为全景图（PNG/WebP 无损）；内置三场景全景拼接算法（动漫相位相关 / 风景 AKAZE / 真人运动掩码 AKAZE），自动场景分类与垃圾帧检测
- **AI 提升画质**：对已生成的动图/全景图结果一键运行 AI 超分辨率处理（Real-ESRGAN，×2/×4，写实或动画风格模型可选），可选 GFPGAN 人脸修复；提供提升前后对比视图及分辨率/文件大小变化；按已配置的计算设备运行（GPU 或 CPU），GPU 初始化失败时自动回退到 CPU
