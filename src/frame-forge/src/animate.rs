...
pub fn encode_gif(_frames: &[DynamicImage], _fps: u16, _loop_count: u16) -> anyhow::Result<Vec<u8>> {
    ...
    let delay = (100u16).saturating_div(_fps.max(1)).max(2);
...
pub fn encode_webp_anim(frames: &[DynamicImage], fps: u16, _loop_count: u16) -> anyhow::Result<Vec<u8>> {
    ...
    let delay_ms = (1000i32).saturating_div(fps.max(1) as i32).max(10);

    let mut encoder = webpx::AnimationEncoder::with_options(w, h, false, 0)?;
    encoder.set_lossless(true);
    for (i, img) in frames.iter().enumerate() {
        let rgba = pad_to_canvas(img, w, h);
        encoder.add_frame_rgba(&rgba, delay_ms * i as i32)?;
    }
    Ok(encoder.finish(delay_ms * frames.len() as i32)?)
}

/// Timestamp-driven WebP animation with first/last frame using average gap.
pub fn encode_webp_timed(frames: &[DynamicImage], timestamps: &[u64], _loop_count: u16) -> anyhow::Result<Vec<u8>> {
    ...
    // Build absolute positions: first gap = avg, middle = actual, last gap = avg
    ...
    // finish controls last frame duration: end - last_ts
    let end_ts = positions[n - 1] + avg_gap_ms as i32;
    Ok(encoder.finish(end_ts)?)
}
...
// ── Timestamp-driven GIF (uses same timing logic as WebP timed version) ─────
