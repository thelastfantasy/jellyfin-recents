...
Usage (called from run_demo.sh):
  python3 gen_report.py <fixtures_dir> <rust_output_dir> <legacy_rust_dir> <loftr_output_dir> <python_output_dir> <report_html>
...
def build_score_table(legacy_data: dict, rust_data: dict, loftr_data: dict, py_data: dict) -> str:
    ...
    all_scenes     = sorted(set(
        ...
        list(loftr_by_name.keys()) + list(py_by_name.keys())
    ))
    ...
    for scene in all_scenes:
        ...
        rs = r.get("ssim"); ls = leg.get("ssim"); lts = loft.get("ssim"); ps = p.get("ssim")
        ...
        rows += f"""
    <tr>
      <td rowspan="4"><strong>{scene}</strong>{mode_badge}<br><small style="color:#666">{meta['desc']}</small></td>
    ...
    <tr class="summary-row">
      ...
      <td>Legacy: {fmt(ls_sum.get('ssim_mean'),4)}<br>LightGlue: {fmt(rs_sum.get('ssim_mean'),4)}<br>LoFTR: {fmt(lts_sum.get('ssim_mean'),4)}<br>Python: {fmt(ps_sum.get('ssim_mean'),4)}</td>
      <td>Legacy: {fmt(ls_sum.get('color_de_mean'),2)}<br>LightGlue: {fmt(rs_sum.get('color_de_mean'),2)}<br>LoFTR: {fmt(lts_sum.get('color_de_mean'),2)}<br>Python: {fmt(ps_sum.get('color_de_mean'),2)}</td>
      <td>Legacy: {fmt(ls_sum.get('rmse_mean'),2)}<br>LightGlue: {fmt(rs_sum.get('rmse_mean'),2)}<br>LoFTR: {fmt(lts_sum.get('rmse_mean'),2)}<br>Python: {fmt(ps_sum.get('rmse_mean'),2)}</td>
      <td></td>
    </tr>"""
...
def generate(fixtures_dir: Path, rust_dir: Path, legacy_dir: Path, loftr_dir: Path, py_dir: Path, out_html: Path):
    ...
    score_table = build_score_table(legacy_data, rust_data, loftr_data, py_data)
...
<div class="subtitle">
  ...
  Generated: {now} &nbsp;|&nbsp; Branch: feature/009-frame-export-stitch &nbsp;|&nbsp; Engines: Legacy AKAZE · LightGlue v2 · EfficientLoFTR · Python AKAZE
</div>
...
def main():
    if len(sys.argv) < 7:
        print(
            f"Usage: {sys.argv[0]} <fixtures_dir> <rust_output_dir> <legacy_rust_dir> <loftr_output_dir> <python_output_dir> <report_html>",
            file=sys.stderr,
        )
        sys.exit(2)
    generate(
