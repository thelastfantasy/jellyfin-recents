...
- **AI upscale**: From any exported GIF/panorama result, run one-click AI super-resolution (Real-ESRGAN, ×2/×4, photo-realistic or anime-tuned model variants) with optional GFPGAN face restoration; before/after comparison view shows the resolution and file-size delta; runs on whichever compute device is configured (GPU or CPU), with automatic CPU fallback on GPU failure
