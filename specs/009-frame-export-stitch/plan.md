...
## Phase 3: 拼接质量�f�分测试基础设施（US9）

### 测试目录结构

```text
tests/stitch-eval/
├── score.py               # 主评分脚本：接收图像对目录 → JSON 评分卡
├── gen_synthetic.sh       # FFmpeg 裁切合成测试序列（平移 Pan 仿真序列）
└── thresholds.json        # 阈值配置（SSIM/接缝/色差/RANSAC/RMSE）

src/frame-forge/src/stitch/
├── stitch_anime.rs        # 含 #[cfg(test)] 质量测试模块
├── stitch_landscape.rs    # 含 #[cfg(test)] + RANSAC 内点率测试
└── stitch_liveaction.rs   # 含 #[cfg(test)] + RMSE 测试
```
...
| 指标 | 含义 | 通过阈值 | 适用路径 |
|------|------|------|------|
| SSIM | 结构相似度（拼接结果 vs 参考图） | ≥ 0.80 | A/B/C 关键帧拼接 |
| 接缝梯度跳变 | 接缝处 5px 范围内梯度均值之差 | ≤ 25.0 | A/B/C 所有路径 |
| 色差 ΔE | L*a*b* 未校正区域色彩漂移均值 | ≤ 10.0 | A/B/C 所有路径 |
| RANSAC 内点率 | 内点数 / 总匹配数 | ≥ 0.40 | B/C 路径（OpenCV Homography） |
| RMSE | 对应点重投影误差 | ≤ 5.0 px | B/C 路径 |

### 测试数据集准备（详见 research.md）

- **场景 A**（动漫）：FFmpeg 裁切合成平移序列：`gen_synthetic.sh` 把源图像水平裁切出 4 个子图（每次偏移 N px），已知真值偏移，Phase Correlation SSIM 应 ≥ 0.90
- **场景 B**（风景）：SEAGULL 数据集（风景全景缝合）：`gh api repos/.../contents` 下载图像对，目标 SSIM ≥ 0.80
- **场景 C**（真人）：UDIS-D 数据集或 Walking Tour FFmpeg 裁切序列：目标 SSIM ≥ 0.80、RANSAC 内点率 ≥ 0.40
...
opencv-python>=4.9
numpy>=1.24
scikit-image>=0.22  # structural_similarity (SSIM)
```

不受 Docker 限制，开发者在本地虚拟环境运行：`uv run --with opencv-python,scikit-image python tests/stitch-eval/score.py`

### Rust 测试依赖（添加到 Cargo.toml [dev-dependencies]）

```toml
[dev-dependencies]
image = "0.25"      # 已生产依赖，可直接用于测试
serde_json = "1"    # 读取 thresholds.json
```

---

## Phase 4: Intel Arc A310 GPU 透传配置（WSL2 + Docker Desktop）

### 宿主机准备

Arc A310 在 WSL2 中需要 Intel 官方 Windows 驱动（≥ 31.0.101.4887）才能将 `/dev/dri/renderD128` 暴露给 WSL2。

```
1. 下载并安装：Intel Arc & Iris Xe Graphics Driver for Windows
   https://www.intel.com/content/www/us/en/products/sku/227957/intel-arc-a310-graphics/downloads.html
   （安装后重启）

2. 验证 WSL2 内可见：
   ls /dev/dri/
   # 预期输出：card0  card1  renderD128  (renderD128 = Arc A310 计算节点)
```

### Docker 启动命令（含 GPU 透传）

```bash
MSYS_NO_PATHCONV=1 docker run -d --name jellyfin-dev \
  -p 8600:8096 \
  -v jellyfin-config:/config \
  -v jellyfin-cache:/cache \
  -v "d:/Dev/jellyfin-recents/demo:/media/demo" \
  -e JELLYFIN_WEB_DIR=/jellyfin/jellyfin-web \
...
`--group-add render` 确保容器进程可读 `/dev/dri/renderD128`（Intel 计算节点权限组）。

### 容器内 OpenCL 运行时

在 `Makefile` build-frame-forge 的 Docker 层添加：

```dockerfile
RUN apt-get install -y intel-opencl-icd clinfo ocl-icd-libopencl1
```

验证命令（容器内）：

```bash
docker exec jellyfin-dev clinfo | grep -E "Platform|Device|Arc"
# 预期：Intel OpenCL + "Intel Arc A310 Graphics" device
```
...
| 变量 | 含义 |
|------|------|
| `FRAME_FORGE_TEST_GPU=1` | 激活需要 GPU 的 Rust `#[ignore]` 测试 |
| `FRAME_FORGE_REQUIRE_GPU=1` | 启动时若无 OpenCL 则 panic（CI 显式强制验证） |

CI 中不设这两个变量 → GPU 测试自动跳过，不阻塞构建。
